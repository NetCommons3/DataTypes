 ...
 */
class DataTypeChoiceFixtureValidateTest extends NetCommonsValidateTest {
 ...
 * @var array
 */
	protected $_modelName = 'DataTypeChoice';
 ...
 */
	public function dataProviderValidationError() {
		$data['DataTypeChoice'] = (new DataTypeChoiceFixture())->records[0];

		return array(
			array($data, 'language_id', 'aaa', __d('net_commons', 'Invalid request.')),
			array($data, 'key', '', __d('net_commons', 'Invalid request.')),
			array($data, 'data_type_key', '', __d('net_commons', 'Invalid request.')),
			array($data, 'name', '', __d('net_commons', 'Invalid request.')),
			array($data, 'code', '', __d('net_commons', 'Invalid request.')),
